...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    ...
    if ($upload_contract_pdf_name !== "") {
        ...
        if (isset($_FILES['upload_contract_pdf'])) {
            ...
            move_uploaded_file($upload_contract_pdf_tmp, "files/shopsContracts/pdf_files/" . iconv('utf-8', 'windows-1256',$upload_contract_pdf_name));
        }
    }
    ...
    if ($upload_id_pdf_name !== "") {
        ...
        if (isset($_FILES['upload_id_pdf'])) {
            ...
            move_uploaded_file($upload_id_pdf_tmp, "files/shopsContracts/pdf_files/" . iconv('utf-8', 'windows-1256', $upload_id_pdf_name));
        }
    }
    ...
    if ($upload_mandate_pdf_name !== "") {
        ...
        if (isset($_FILES['upload_mandate_pdf'])) {
            ...
            move_uploaded_file($upload_mandate_pdf_tmp, "files/shopsContracts/pdf_files/" . iconv('utf-8', 'windows-1256', $upload_mandate_pdf_name));
        }
    }


    $sql = "UPDATE shops SET renter_id='$renter_SSN' , renting_price='$renting_price'" . $imagesSqlText . " WHERE id='$shop_id'";
    $query = mysqli_query($connect, $sql);
    if ($query) {

        //echo "Shop information is updated";
    } else {
        echo "There is problem in updating the shop information : " . $sql;
    }
    ...
    for ($i = 0; $i < $payments_number; $i++) {
        ...
        $sql_select = "SELECT payment_price FROM shops_payments WHERE shop_id = '$shop_id' && payment_order = '$payment_order'";
        $query_select = mysqli_query($connect, $sql_select);

        if (mysqli_num_rows($query_select) > 0) {

            $sql_update = "UPDATE shops_payments SET payment_price = '$payment_price' , due_date = '$payment_due_date' WHERE shop_id = '$shop_id' && payment_order = '$payment_order'";
            ...
            if ($query_update = mysqli_query($connect, $sql_update)) {
                ...
                if ($payment_order == $payments_number) {
                    $sql_clean = "DELETE FROM shops_payments WHERE shop_id = '$shop_id' && payment_order > $payment_order";
            ...
            $sql_insert = "INSERT INTO shops_payments (payment_price, due_date, status, shop_id, payment_order) VALUES ('$payment_price','$payment_due_date', 0, '$shop_id', '$payment_order')";
...
?>
    ...
    <!-- <meta http-equiv="refresh" content="0;url=<?php echo $_SERVER['PHP_SELF'] . "?id=" . $shop_id; ?>" /> -->
